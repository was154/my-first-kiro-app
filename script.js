...
function createCourseCard(course) {
    ...
    card.innerHTML = `
        <div class="course-header">
            ...
            <div class="course-distance">${course.distance}</div>
        </div>
        ...
        <div class="course-mysteries">
            <i class="fas fa-search"></i>
            <span>${course.mysteries}個の謎解きあり</span>
        </div>
        <p class="course-description">${course.description}</p>
        <button class="course-button" onclick="viewCourseDetails(${course.id})">
...
function viewCourseDetails(courseId) {
    ...
    if (course) {
        ...
        // 実際の実装では、詳細ページにリダイレクトまたはモーダルを表示
    }
}
...
// アニメーション対象の要素を監視
document.addEventListener('DOMContentLoaded', function() {
    const animateElements = document.querySelectorAll('.feature-card, .feature-item');
...
function updateStats() {
    const totalDistance = coursesData.reduce((sum, course) => {
        return sum + parseFloat(course.distance);
    }, 0);
    
    const totalMysteries = coursesData.reduce((sum, course) => {
        return sum + course.mysteries;
    }, 0);
    
    console.log(`総距離: ${totalDistance.toFixed(1)}km`);
    console.log(`総謎解き数: ${totalMysteries}個`);
}